...
<section class="content">
  ...
  <div class="box">
    ...
    <div class="box-body">
      ...
      <form method="GET" action="{{ route('sponsoring.search') }}">
        <div class="box box-default">
          ...
          <div class="box-footer">
            ...
            <a href="{{ route('sponsoring.plan.send') }}" class="btn btn-primary">
              <span class="glyphicon glyphicon-share-alt" aria-hidden="true"></span>
                @lang('lang.task.planifier.send.email')
            </a>
          </div>
        </div>
      </form>
        ...
        <div class="row">
          <div class="col-sm-12">
            <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
              ...
              <tbody>
                @foreach ($subscribers as $indexKey =>$subscriber)
                @if($indexKey % 2 )
                ...
                <tr role="row" class="odd">
                  ...
                  <td class="sorting_1"> {{$subscriber->username}}</td>
                  <td class="hidden-xs">{{ $subscriber->prenom  }}</td>
                  <td class="hidden-xs">{{ $subscriber->email  }}</td>
                  <td class="hidden-xs">{{ $subscriber->number  }}</td>
                  <td class="hidden-xs">
                      <a href=" {{ route('sponsoring.email.form', ['id' => $subscriber->id]) }}" class="btn btn-danger">@lang('lang.send.message')</a>
                  </td>
                </tr>
                @endforeach
              </tbody>
